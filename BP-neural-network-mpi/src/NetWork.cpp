...
void NetWork::process(const double inputs[], double* outputs[]) //测试的时候直接运行，不用反向
{
    for (int i = 0; i < mNumHiddenLayers + 1; i++) {
...
double NetWork::backActive(double x) {//导数
...
bool NetWork::training(const double inputs[], const double targets[]) {
    ...
    trainUpdate(inputs, targets);//向前传播
    ...
    for (int i = mNumHiddenLayers; i >= 0; i--) {
        ...
        trainNeuronLayer(curLayer, prevOutActivations, prevOutErrors);//反向传播
    }
