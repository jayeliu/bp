...
bool dataLoader::readIndex(int* label) {
...
bool dataLoader::readImage(char imageBuf[]) {
...
bool dataLoader::read(int* label, char imageBuf[]) {
	if (readIndex(label)) {
...
void dataLoader::movepoint(int offset)//move the file piont
{
	if (mImageFile.is_open()) {
		mImageFile.seekg(mImageStartPos+offset*mImageLen);
	}

	if (mLabelFile.is_open()) {
		mLabelFile.seekg(mLableStartPos+offset*mLabelLen);
	}
}
